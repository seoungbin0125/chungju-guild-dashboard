...
기존 Lovely 프로젝트와 별개로 운영하는 충주시(Scania 4) 전용 대시보드입니다. 전투력, 토벌전 참여/점수, **Scania 4 서버 투력·토벌 순위 효율**, 개인 대항전 주간 이력과 상승률, 콘텐츠 매칭, 공략 게시판, 실시간 길드 광장을 포함합니다.

## v2.5.2 MGF 지난주 점수 이월 판별

- 일요일에 저장한 최종 점수와 월요일 이후 MGF 원본 점수가 그대로 같으면 **이번 주 미참여(0점)** 로 처리
- MGF 원본 숫자는 `sourceTobeolValue`에 그대로 보존하고, 화면·참여 인원·총점·서버 토벌 순위에는 `currentWeekTobeolValue`만 사용
- 원본 점수가 달라지는 순간 이번 주 새 참여 점수로 자동 전환
- 이미 이번 주 참여로 판정된 뒤 우연히 지난주와 같은 값이 되어도 다시 미참여로 되돌리지 않음
- 구버전 데이터도 지난주 점수와 현재 원본이 같으면 화면에서 즉시 이월 점수로 보정
- Windows/Mac 업로드 파일이 원격 자동수집 커밋을 먼저 합치고, push 경합 때 한 번 더 동기화하도록 수정

## v2.5.1 토벌 주차 수정

- 토벌 주차를 실제 규칙대로 **월요일 시작·일요일 종료**로 변경
- 월요일 초기화 점수가 지난주 최종 점수로 들어가던 오류 수정
- 기존 이력에 잘못 분류된 월요일 기록은 지난주 비교에서 자동 제외
- 지난주 일요일 기록을 우선하고, 없으면 그 주의 마지막 수집본을 사용
- 이번 주 누적과 지난주 점수의 차이·비율을 길드원별로 표시
- 일요일 23:20·23:40·23:55 KST 추가 수집으로 마감 기록 보존 강화

## v2.5.0 추가 기능

- 효율 지표를 길드 내부 점수 모델에서 `서버 전투력 순위 대비 서버 토벌 순위 향상률`로 변경
- 직업 보정과 기대점수를 제거하고 서버 투력 순위·서버 토벌 순위·순위차를 그대로 표시
- 숨겨져 있던 버츄얼 광장을 메인 `광장` 탭으로 공개
- 길드 캐릭터 선택, 맵 이동, 빠른 채팅, 공격·돌진·회복·실드와 5초 감옥 놀이 지원
- 광장 접속 위치와 최근 채팅 80개를 Firebase에 실시간 저장
- Windows GitHub 계정 충돌을 해결하는 `FIX_GITHUB_LOGIN.bat` 추가
- 광장 채팅 권한 오류 때만 실행하는 `DEPLOY_FIREBASE_RULES.bat` 추가

## v2.4.0 추가 기능

- 현재 전투력과 토벌 점수로 효율지수·기대점수·등급을 계산하는 `효율` 탭 추가
- 길드원 30명 효율표와 직업별 기준 멤버 표시
- `대항전 → 개인 점수 입력`에서 이번 주와 지난주 개인 점수를 직접 입력
- 입력값을 목요일 낮 12시 시작 주차별로 Firebase에 저장해 다음 주 비교에 자동 연결
- Firebase를 사용할 수 없을 때는 현재 브라우저에 임시 저장
- 접근성 진단 파일의 Windows PowerShell 한글 정규식 오류 수정

## v2.3.0 수정 사항

- `Guild Conquest`는 토벌전, `Guild War`는 대항전으로 구분하며 서로의 점수를 대체하지 않음
- 대항전 주차 경계를 목요일 낮 12시(KST)로 적용
- 공개 페이지의 `previous_damage`는 지난주 값이 아니라 직전 제출값이므로, 제출 시각에 맞는 주차에만 기록
- 대시보드 전 메뉴에서 원본 링크와 수집 필드·배포 방식 설명을 숨겨 길드원용 화면으로 정리
- `PUSH_UPDATE_TO_GITHUB.command`가 원격 변경을 먼저 합쳐 `fetch first` 오류를 줄임
...
- 대항전 개인 점수를 목요일 낮 12시 시작 주차별로 `data/guild-war-history.json`에 보관
...
브라우저에서 `http://localhost:8080`을 엽니다. 이미 GitHub에 연결한 기존 폴더에 업데이트를 덮어쓰려면 압축을 푼 폴더의 `APPLY_TO_WORK_FOLDER.command`를 실행하세요. 기존 프로젝트를 자동으로 찾지 못하면 폴더를 터미널 창으로 끌어다 놓으면 됩니다.
...
새 구조는 날짜가 아니라 **월요일 시작 주차 키**로 저장합니다.

- 월요일~토요일 MGF 값: 해당 주차의 `current_week_live`
- 일요일 MGF 값: 해당 주차의 `current_week_final`
- 지난주 비교: 지난주 일요일 수집본을 우선 사용
- 일요일 수집본이 없으면 지난주 범위 안의 마지막 수집본을 사용하고 `최근 기록`으로 표시
- 잘못 분류된 다음 주 월요일 데이터는 지난주 비교에서 자동 제외
- 차이: `이번 주 현재 누적 - 지난주 최종`, 비율: `차이 / 지난주 × 100`
- 전투력만 7일 전 목표일 근처 ±2일의 실제 수집일과 비교

`data/history.json`에 주차·확정 여부·원본 기준일이 함께 저장됩니다. 처음 설치한 주에는 지난주 이력이 없으며 지난주 범위의 수집 이력이 생긴 뒤 비교가 표시됩니다.
...
- 지난주: 저장된 지난 주차 스냅샷을 우선 사용하고, 직전 제출값은 그 제출 시각이 지난주 범위일 때만 보강에 사용
...
공개 페이지에 없는 길드원의 대항전 점수를 OCR 없이 가져오려면 게임 화면이 ADB 텍스트로 노출되는지 먼저 확인해야 합니다. 게임에서 대항전 개인 점수 목록을 연 뒤 `CHECK_ADB_GUILD_WAR.command`를 실행하면 `ADB 가능` 또는 `ADB 불가능`만 표시합니다.

윈도우에서 LD플레이어를 사용하는 경우에는 LD플레이어 설정에서 ADB 디버깅을 `로컬 연결`로 설정하고, 개인 점수 목록을 연 상태에서 `CHECK_LDPLAYER_GUILD_WAR.bat`를 실행합니다. 이 검사는 최초 한 번만 필요하며, `ADB 가능`이 나오면 이후 자동 실행·화면 진입·스크롤·기록 단계를 연결할 수 있습니다.

검사기는 실행 중인 LD플레이어 인스턴스를 `ldconsole`로 우선 확인하므로 일반 `adb devices` 목록에 에뮬레이터가 나타나지 않는 설치 환경도 지원합니다.

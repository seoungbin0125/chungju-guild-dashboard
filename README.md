...
기존 Lovely 프로젝트와 별개로 운영하는 충주시(Scania 4) 전용 대시보드입니다. 전투력, 토벌전 참여/점수, **투력 순위와 토벌 순위 비교**, 지난주 토벌 이력, 대항전·수련장·보스대전 매칭, 공략 게시판을 포함합니다.

## v2.1.0 추가 기능

- 길드 내 전투력 순위와 토벌 순위를 길드원별로 나란히 표시
- `전투력 순위 - 토벌 순위`로 토벌 순위 상승·하락 폭 표시
- 토벌 점수 0은 억지로 꼴찌 순위를 주지 않고 `미참여` 처리
- 동점은 공동 순위 방식으로 계산
- 수집된 현황 화면과 실시간 토벌 조회 화면에서 모두 비교 가능
- 토벌 순위, 투력 순위, 토벌 순위 상승 폭 기준 정렬

GitHub 업로드부터 자동 수집과 Cloudflare 서버 배포까지는 `GITHUB_AND_SERVER_DEPLOY.md`에 화면별 설정값을 정리했습니다. 빈 GitHub 저장소를 만든 후 `PUSH_TO_GITHUB.command`를 실행하면 최초 업로드를 진행할 수 있습니다.
...
기존 코드는 HTML을 한 줄의 텍스트로 만든 다음 직업/레벨 뒤에 나온 숫자 중 **마지막 숫자**를 전투력으로 골랐습니다. 현재 화면에서는 마지막 숫자가 토벌 점수이므로 전투력에 토벌 점수가 들어갔습니다. 그 뒤 서버 전체 토벌 랭킹을 최대 20페이지 읽고 닉네임만으로 다시 결합해 다음 문제가 추가됐습니다.
...
v2.0.0은 길드원 행의 `data-bp`와 `data-gb` 정수를 직접 읽습니다. 길드원 수와 길드 전체 토벌 합계까지 검증하고, 검증에 실패하면 잘못된 JSON을 저장하지 않습니다. 실시간 토벌 조회도 길드 상세 1회 요청으로 끝납니다.

정기 수집은 길드 상세 1회와 콘텐츠 페이지 3회만 요청하도록 줄였습니다. 원본 사이트 정책과 과도한 요청 방지를 위해 수집 주기를 불필요하게 짧게 설정하지 마세요.
...
따라서 이 프로젝트는 없는 필드를 0점이나 미실행으로 추정하지 않습니다. 화면에 **MGF 원본 미제공**으로 표시하고, 대항전은 실제로 제공되는 매칭 그룹만 기록합니다. 나중에 합법적으로 접근 가능한 원본 필드/API가 추가되면 `scoreValue`를 자동으로 인식하도록 데이터 구조는 준비되어 있습니다. OCR은 사용하지 않습니다.
...
`.github/workflows/collect.yml`이 매일 KST 02:10에 실행됩니다. 파서 테스트가 먼저 통과해야 데이터 수집과 커밋을 진행합니다. `sourceDataDate`가 실제 MGF 기준일로 함께 저장되므로 실행일과 원본 기준일이 다른 상황도 구분할 수 있습니다.
